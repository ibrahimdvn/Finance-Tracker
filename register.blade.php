...
<body>
    <div class="auth-container">
        <div class="logo">
            <svg viewBox="0 0 316 316" xmlns="http://www.w3.org/2000/svg" width="54" height="54">
        ...
        <div class="auth-desc">Register to start tracking your finances</div>
        <form method="POST" action="{{ route('register') }}" class="auth-form">
            @csrf
            <label for="name">Name</label>
            <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name">
            @error('name')<div style="color:#ef4444; font-size:0.97rem; margin-bottom:8px;">{{ $message }}</div>@enderror
            ...
            @error('password_confirmation')<div style="color:#ef4444; font-size:0.97rem; margin-bottom:8px;">{{ $message }}</div>@enderror
